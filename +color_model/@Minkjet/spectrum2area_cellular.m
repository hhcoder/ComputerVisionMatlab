function mreturn = spectrum2area_cellular( obj, in_spectrum, ctrl )
    opt_option = optimset(...
        'DiffMaxChange', 0.5,...
        'DiffMinChange', 0.01,...
        'DerivativeCheck', 'off',...
...
%         for j = 1:2
            for k=1:size(op_key,2)
                ...
                pred_func = @(guess_dc)forward_model_cellular( ...
                    guess_dc, ...
                    in_spectrum(i,:), ...
                    obj, ...
                    op_key{k} );
...
function ret = forward_model_cellular( guess, input_spectrum, inkjet_obj, op_key )

    pred_spectrum = inkjet_obj.area2spectrum_cellular( guess );
